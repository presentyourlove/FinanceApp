...
3. **資料隱私與合規 (Data Privacy & Compliance)**：
...
4. **專案結構與命名 (Project Structure & Naming)**：
...
5. **Git 與版本控制 (Git & Version Control)**：
...
6. **DevOps 與部署 (DevOps & Deployment)**：
...
7. **雲端原生架構 (Cloud Native Architecture)**：
...
8. **可觀測性與監控 (Observability & Monitoring)**：
...
9. **文件準則 (Documentation Guidelines)**：
...
1. **前端開發指南 (Frontend Development Guidelines)**：適用於 Web 與 App 開發。
...
2. **後端開發指南 (Backend Development Guidelines)**：適用於 Node.js, Java Spring, Go, Python Flask/FastAPI 等：
...
4. **資料庫開發準則 (Database Development Guidelines)**：適用於 SQL 與 NoSQL：
...
5. **行動 App 開發準則 (Mobile App Guidelines)**：適用於 Android/iOS (Native, Flutter, React Native)：
...
6. **企業系統開發準則 (Enterprise System Guidelines)**：
...
7. **嵌入式程式開發準則 (Embedded Systems Guidelines)**：適用於 IoT, MCU, Firmware：
...
1. **Python 開發準則 (Python Development Guidelines)**：
    * **語法與風格**：
        * 遵循 **PEP 8** 官方風格指南。
        * 使用 **Black** 進行自動排版。
...
2. **JavaScript / TypeScript 開發準則 (JS/TS Development Guidelines)**：
    * **語法與風格**：
...
3. **Java 開發準則 (Java Development Guidelines)**：適用於 Spring Boot：
    * **語法與風格**：
...
4. **Go (Golang) 開發準則 (Go Development Guidelines)**：
    * **語法與風格**：
...
5. **C# (.NET) 開發準則 (C#/.NET Development Guidelines)**：
    * **語法與風格**：
